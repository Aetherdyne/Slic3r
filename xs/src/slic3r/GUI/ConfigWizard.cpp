...
PagePrinters::PagePrinters(ConfigWizard *parent, wxString title, wxString shortname, const VendorProfile &vendor, unsigned indent)
    : ConfigWizardPage(parent, std::move(title), std::move(shortname), indent)
{
    ...
    for (const auto &family : families) {
        ...
        std::cerr << "reading family: " << family << std::endl;
    ...
    , p(new priv(this))
{
    ...
    p->page_fff = new PagePrinters(this, _(L("Prusa FFF Technology Printers")), "Prusa FFF", vendor_prusa, 0);
